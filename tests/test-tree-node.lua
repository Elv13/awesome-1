...
local current_drawins = setmetatable({}, {__mode = "v"})
...
table.insert(steps, function()
    ...
    for _, color in ipairs(colors) do
        ...
        assert(w.drawin)
        table.insert(current_drawins, w.drawin)
        table.insert(wiboxes, w)
        if root1.first_child then
            table.insert(nodes, _tree_node {
...
table.insert(steps, function()
    ...
    if #current_drawins > 0 then return end
...
table.insert(steps, function()
    ...
    while n1 do
        ...
        if n1.type == "client" then
            ...
            assert(type(n1.honor_size_hints) == "table")
            assert(n1.honor_size_hints.maximum_width == n2.honor_size_hints.maximum_width)
