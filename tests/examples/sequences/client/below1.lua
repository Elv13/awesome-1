...
module.add_event("Spawn some apps", function()
  --DOC_HIDE_END
  -- Spawn some clients.
  awful.spawn("client", {name = "client #1", below = false})
  awful.spawn("client", {name = "client #2", below = true })
  awful.spawn("client", {name = "client #3", below = true })
  awful.spawn("client", {name = "client #4", below = false})
  --DOC_HIDE_START
end)

module.display_stacking()

module.add_event('Lower "client #1"', function()
  --DOC_HIDE_END
  --DOC_NEWLINE
...
module.add_event('Move "client #4" to the below layer and #3 to above', function()
  --DOC_HIDE_END
  --DOC_NEWLINE

  -- Move "client #1" to the below layer.
  client.get()[3].below = true -- now "client #4" due to the :raise() below
  client.get()[2].above = true -- now "client #3" due to the :raise() below

  --DOC_HIDE_START
  --assert(client.get()[3].below and client.get()[2].above)
end)
