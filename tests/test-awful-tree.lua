...
table.insert(steps, function()
    ...
    tree1.first_child.geometry = geo

    for _, side in ipairs { "left", "right", "top", "bottom" } do
        assert(geo[side] == tree1.first_child.geometry[side])
    end
    ...
    tree1.first_child.gaps = gaps

    for _, side in ipairs { "left", "right", "top", "bottom" } do
        assert(gaps[side] == tree1.first_child.gaps[side])
    end

    local new_geo = tree1.first_child.effective_geometry
    ...
    tree1.first_child.placement = function(obj, args)
        print("\n\nPLACE ", obj.geometry.width, args.pretend, args.bounding_rect.width)
        was_called = true
        return {
            x      = 1000,
            y      = 1001,
