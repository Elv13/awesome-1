...
for node in awful.tree.iterate_next(tree, true) do
    local previous_node = node.previous_sibling
    local lbl = (not previous_node)
        and "nil"
        or (previous_node.label and '"'..previous_node.label..'"')
        or "N/A"

    labels[node] = "<i><span color='"..color.."'>previous_sibling:</span></i> "..lbl
end
